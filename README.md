...
Spool collects every Claude Code, Codex CLI, Gemini CLI, Antigravity CLI, OpenCode, and Grok Build session you've ever had into a sidebar of projects you can browse, pin, and revisit. Press ⌘K to search across the whole archive.
...
- **Session indexing** — watches Claude/Codex/Gemini/Antigravity session dirs, OpenCode's SQLite database, and Grok Build's `~/.grok/sessions/` directory in real time, including profile-based paths like `~/.claude-profiles/*/projects`, `~/.codex-profiles/*/sessions`, Gemini/Antigravity project temp dirs under `~/.gemini/tmp/*/chats` and `~/.gemini/antigravity-cli/brain/`, `~/.local/share/opencode/opencode.db`, and `~/.grok/sessions/{cwd}/{session_id}/chat_history.jsonl`
