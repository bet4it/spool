...
export function upsertSession(
  ...
  if (existing) {
    ...
    `).run(
      opts.title,
      parentSessionUuid,
      opts.filePath, opts.startedAt, opts.endedAt,
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    opts.projectId, opts.sourceId, opts.sessionUuid,
    parentSessionUuid,
    opts.filePath, opts.title,
...
export function getStatus(db: Database.Database): StatusInfo {
  ...
  const grokRow = counts.find(r => r.name === 'grok')
