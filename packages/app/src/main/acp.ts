...
function agentIdToSource(agentId: string): SessionSource | null {
  if (agentId === 'claude' || agentId === 'codex' || agentId === 'gemini' || agentId === 'antigravity' || agentId === 'opencode' || agentId === 'grok') return agentId
  return null
}
...
export class AcpManager {
  ...
  private buildPrompt(userQuery: string): string {
    ...
    const systemBody = [
      'You have access to a local knowledge base called Spool that indexes the user\'s AI coding sessions (Claude Code, Codex CLI, Gemini CLI, Antigravity CLI, OpenCode, Grok Build).',
      '',
      'The database is at ~/.spool/spool.db (SQLite with FTS5). You can query it directly with the `sqlite3` CLI.',
      '',
      '── Schema ──',
      '  sources(id, name TEXT, base_path TEXT)  -- "claude", "codex", "gemini", "antigravity", "opencode", or "grok"',
      ...
      '- If the user names a specific source (claude/codex/gemini/antigravity/opencode/grok), only return results from that source unless they explicitly ask for cross-source search.',
