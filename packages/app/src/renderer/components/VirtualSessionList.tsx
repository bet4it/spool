...
export type SessionListRow =
  ...
  | { kind: 'session'; id: string; session: Session; pinned?: boolean; showProject?: boolean; bucket?: BucketKey; dateIso?: string; headerId: string | null; treeDepth?: number; treeAncestorIds?: string[]; treeChildCount?: number }
...
// GroupedVirtuoso counts each group header as one of the data items it
// manages (totalCount = sum(groupCounts) + numGroups). The data array must
// therefore contain one placeholder per group at the group's first index —
// `groupContent` renders the header for that index, `itemContent` no-ops it.
// Without the placeholder, data.length < expected totalCount, and inserting a
// row mid-group leaves the new item unrendered (the missing-9th-row bug).
...
export default function VirtualSessionList({
  ...
  // Interleave a `null` group-header placeholder at the start of each group
  // so `data.length` matches GroupedVirtuoso's expected totalCount — see
  // StickyItemData. The groupContent callback renders the actual header.
  ...
  }, [closed, collapsibleSections, onCopySessionId, onOpenSession, onPinChange, onShare, openTreeNodes, toggleHeader, toggleTreeNode])
    ...
    return (
      <div className="relative flex-1 min-h-0">
        <GroupedVirtuoso
          ...
          computeItemKey={(_index, row) => (row ? row.id : `group-${_index}`)}
